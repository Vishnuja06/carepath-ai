...
test('smoke test - app loads referral workspace', async ({ page }) => {
  await page.goto('/');

  // Header
  await expect(page.getByRole('heading', { name: 'CarePath AI' })).toBeVisible();

  // Patient-need form
  await expect(page.getByText('Patient need')).toBeVisible();
  await expect(page.getByText('Specialty / service needed')).toBeVisible();
  await expect(page.getByPlaceholder('e.g. 110002')).toBeVisible();

  // Empty state before a search is run
  await expect(page.getByText('Start a referral')).toBeVisible();
});

test('smoke test - trust overview strip renders tiers', async ({ page }) => {
  await page.goto('/');

  // trust_overview query populates a summary line with the tier counts
  await expect(page.getByText(/facilities scored for information trust/i)).toBeVisible();
});
